    ...
    def run(self, optimizers, X_train, y_train, X_test, y_test):
        ...
        for optimizer in optimizers:
            try:
                ...
                y_train_prob = BaseOptimizer.predict(X_train, train_result.weights, train_result.bias)
                y_train_pred = (y_train_prob >= 0.5).astype(int)
                train_metrics = self.evaluator.calculate_metrics(y_train, y_train_pred, y_train_prob)

                y_test_prob = BaseOptimizer.predict(X_test, train_result.weights, train_result.bias)
